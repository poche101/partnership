    ...
    <div class="flex min-h-screen w-full">
        ...
        <header class="fixed inset-x-0 top-0 z-30 flex h-14 items-center justify-between border-b border-sidebar-border bg-sidebar px-4 text-sidebar-foreground md:hidden">
            <div class="flex items-center gap-2">
                <div class="grid h-8 w-8 place-items-center rounded-md bg-sidebar-primary text-sidebar-primary-foreground">
                    <span class="h-4 w-4"><?php echo $logoSvg; ?></span>
                </div>
        ...
        >
            <div class="flex items-center justify-between px-5 py-5">
                <div class="flex items-center gap-2">
                    <div class="grid h-9 w-9 place-items-center rounded-md bg-sidebar-primary text-sidebar-primary-foreground">
                        <span class="h-5 w-5"><?php echo $logoSvg; ?></span>
                    </div>
                    <div>
            ...
            <nav class="flex-1 space-y-0.5 px-3">
                <?php $__currentLoopData = $navMarkup(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route($row['item']['route'])); ?>"
                       data-close-menu="mobile-nav"
                       class="flex items-center gap-3 rounded-md px-3 py-2 text-sm transition <?php echo e($row['active'] ? 'bg-sidebar-primary text-sidebar-primary-foreground' : 'hover:bg-sidebar-accent'); ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><?php echo $icons[$row['item']['icon']]; ?></svg>
                        <span><?php echo e($row['item']['label']); ?></span>
                    </a>
